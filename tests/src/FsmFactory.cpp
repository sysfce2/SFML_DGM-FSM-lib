...
public:
    ...
    [[nodiscard]] virtual dgm::fsm::loader::StateMap
    loadFromFile(const std::string&) const override
    {
        return states;
    }
};
...
TEST_CASE("[FsmFactory]", "[FsmFactory]")
{
    ...
    factory.registerPredicate(
        "cond1",
        [&](const Blackboard&) -> bool
        {
            cond1_callCnt++;
            return false;
        });
    ...
    SECTION(
        "Can construct minimal FSM with "
    ...
    SECTION("Can deal with empty behavior")
    {
        loader.states = {
            { 0,
              { .transitions = {}, .behaviors = {}, .defaultTransition = 0 } }
        };
